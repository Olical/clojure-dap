...
(defn start!
  ...
  [_]
  (tel/log! :info "Starting malli dev instrumentation")
  (malli-dev/start! {:report (malli-pretty/reporter)})

  (tel/log! :info "Starting nREPL server")
  (let [{:keys [port] :as _server} (nrepl/start-server :handler cider/cider-nrepl-handler)]
    (tel/log! :info ["nREPL server started on port" port])
    (tel/log! :info "Writing port to .nrepl-port")
    (spit ".nrepl-port" port))

  (tel/log! :info "Starting interactive REPL")
  (rr/with-line-reader
